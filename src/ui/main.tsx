...
function App() {
  ...
  return <main className={`shell shell-${activeTab}`}>
    ...
    {activeTab === 'queue' && <section className="tab-panel" role="tabpanel" aria-label="بنك التغريدات وقائمة Queue">
      ...
      <section className="card"><div className="queue">{pagedQueue.map((item) => <QueueRow key={item.id} item={item} selected={selectedQueueIds.includes(item.id)} onToggle={() => toggleQueueItem(item.id)} onAction={queueAction} />)}{!visibleQueue.length && <p className="muted">لا توجد عناصر مطابقة للبحث الحالي.</p>}</div>{visibleQueue.length > 0 && <div className="pagination-controls" aria-label="Pagination Queue"><button onClick={() => setQueuePage((current) => Math.max(1, current - 1))} disabled={safeQueuePage <= 1}>السابق</button><span>صفحة {safeQueuePage} من {queuePageCount}</span><button onClick={() => setQueuePage((current) => Math.min(queuePageCount, current + 1))} disabled={safeQueuePage >= queuePageCount}>التالي</button></div>}</section>
    </section>}
