...
function App() {
  ...
  return <main className={`shell shell-${activeTab}`}>
    <header className="app-header"><div className="brand-lockup"><img className="brand-logo" src={logoUrl} alt="X-Pilot" /><div><span className="eyebrow">LOCAL-FIRST · MV3</span><h1>X-PILOT</h1><p>{t('operation.publishingCenter')}</p></div></div><div className="header-status"><span className="header-live-dot" aria-hidden="true" /><span>{t(`statuses.${runtimeStatus.engineStatus}`)}</span><StatusBadge status={session?.status ?? 'IDLE'}>{t(`statuses.${session?.status ?? 'IDLE'}`)}</StatusBadge></div></header>
    <div className="workspace-switcher premium-switcher"><div className="workspace-switcher-copy"><span className="eyebrow">WORKSPACE ACTIVE</span><strong>{activeWorkspace?.name ?? 'اختر Workspace'}</strong></div><select value={meta?.activeWorkspaceId ?? ''} onChange={(event) => void switchWorkspace(event.target.value)} aria-label="Workspace النشطة">{workspaces.filter((workspace) => !workspace.archived).map((workspace) => <option key={workspace.id} value={workspace.id}>{workspace.favorite ? '★ ' : ''}{workspace.name}</option>)}</select><button className="icon-button workspace-manage-button" onClick={() => setActiveTab('workspaces')} aria-label="إدارة المساحات" title="إدارة المساحات"><Icon name="workspace" /></button></div>
    <nav className="tabs-bar" aria-label={t('nav.operation')}><div className="tabs" role="tablist">
      <div className="nav-group"><span>CONTROL</span><TabButton id="operation" activeTab={activeTab} onSelect={setActiveTab} icon="play" label={t('nav.operation')} /><TabButton id="tests" activeTab={activeTab} onSelect={setActiveTab} icon="check" label={t('nav.startupTests')} /></div>
      <div className="nav-group"><span>CONTENT</span><TabButton id="queue" activeTab={activeTab} onSelect={setActiveTab} icon="bank" label={t('nav.banks')} /></div>
      <div className="nav-group"><span>ACTIVITY</span><TabButton id="sessions" activeTab={activeTab} onSelect={setActiveTab} icon="clock" label={t('nav.sessions')} /><TabButton id="history" activeTab={activeTab} onSelect={setActiveTab} icon="activity" label={t('nav.history')} /></div>
      <div className="nav-group"><span>INSIGHTS</span><TabButton id="analytics" activeTab={activeTab} onSelect={setActiveTab} icon="analytics" label={t('nav.analytics')} /><TabButton id="diagnostics" activeTab={activeTab} onSelect={setActiveTab} icon="diagnostics" label={t('nav.diagnostics')} /></div>
      <div className="nav-group"><span>MANAGE</span><TabButton id="workspaces" activeTab={activeTab} onSelect={setActiveTab} icon="workspace" label={t('nav.workspaces')} /><TabButton id="settings" activeTab={activeTab} onSelect={setActiveTab} icon="settings" label={t('nav.settings')} /></div>
...
function BulkActionsBar({ selectedCount, banks, onAction }: { selectedCount: number; banks: TweetBank[]; onAction: (action: BulkQueueAction, bankId?: string) => void }) { return <div className="bulk-actions"><strong>{selectedCount} عنصر محدد</strong><div className="bulk-buttons"><button onClick={() => onAction('SKIP')}>Skip</button><button onClick={() => onAction('RETRY')}>Retry</button><button onClick={() => onAction('RESET_PENDING')}>Reset to Pending</button><button onClick={() => onAction('MOVE_TOP')}>Move to top</button><button onClick={() => onAction('MOVE_BOTTOM')}>Move to bottom</button><select defaultValue="" onChange={(event) => { if (event.target.value) onAction('ASSIGN_BANK', event.target.value); event.currentTarget.value = ''; }} aria-label="تعيين بنك"><option value="">Assign Bank</option>{banks.filter((bank) => !bank.archived).map((bank) => <option key={bank.id} value={bank.id}>{bank.name}</option>)}</select><button onClick={() => onAction('EXPORT')}>Export selected</button><button className="danger" onClick={() => onAction('DELETE')}>Delete</button></div></div>; }

function QueueRow({ item, selected, onToggle, onAction }: { item: QueueItem; selected: boolean; onToggle: () => void; onAction: (message: RuntimeMessage, success?: string) => Promise<void> }) { return <div className={`queue-row ${selected ? 'queue-row-selected' : ''}`}><input className="queue-select" type="checkbox" checked={selected} onChange={onToggle} aria-label={`تحديد العنصر ${item.position}`} /><span className="position">{item.position}</span><div className="item-main"><div className="preview" dir="auto">{getTweetPreview(item.targetUrl, item.label)}</div><a className="source-link" href={item.targetUrl} target="_blank" rel="noreferrer">فتح رابط التجهيز</a><small>{item.status} · attempts: {item.attempts}{item.lastError ? ` · ${item.lastError}` : ''}</small></div><div className="item-actions"><button aria-label="تحريك لأعلى" onClick={() => void onAction({ type: 'REORDER', itemId: item.id, direction: 'up' })}>↑</button><button aria-label="تحريك لأسفل" onClick={() => void onAction({ type: 'REORDER', itemId: item.id, direction: 'down' })}>↓</button><button aria-label="إعادة المحاولة" onClick={() => void onAction({ type: 'RETRY_ITEM', itemId: item.id })}>↻</button><button aria-label="حذف" onClick={() => void onAction({ type: 'DELETE_ITEM', itemId: item.id })}>×</button></div></div>; }
