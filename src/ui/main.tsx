...
function App() {
  ...
  const toggleVisibleQueue = () => setSelectedQueueIds((current) => visibleQueue.every((item) => current.includes(item.id)) ? current.filter((id) => !visibleQueue.some((item) => item.id === id)) : [...new Set([...current, ...visibleQueue.map((item) => item.id)])]);
  ...
  return <main className={`shell shell-${activeTab}`}>
    ...
    {activeTab === 'queue' && <section className="tab-panel" role="tabpanel" aria-label="بنك التغريدات وقائمة Queue">
      ...
      <section className="card"><div className="section-heading"><h2>Queue</h2><span className="session-progress">{visibleQueue.length} من {state.queue.length} عنصر</span><button onClick={() => void act({ type: 'CLEAR_COMPLETED' })}>Clear Completed</button></div><SearchToolbar filters={queueFilters} onChange={setQueueFilters} workspaces={workspaces} banks={searchData.banks} showStatus /><div className="bulk-select-row"><label className="check"><input type="checkbox" checked={visibleQueue.length > 0 && visibleQueue.every((item) => selectedQueueIds.includes(item.id))} onChange={toggleVisibleQueue} /> تحديد العناصر الظاهرة</label><span>{selectedQueueIds.length} محدد</span></div>{selectedQueueIds.length > 0 && <BulkActionsBar selectedCount={selectedQueueIds.length} banks={banks} onAction={(action, bankId) => void runBulkAction(action, bankId)} />}</section>
      <section className="card"><div className="queue">{visibleQueue.map((item) => <QueueRow key={item.id} item={item} selected={selectedQueueIds.includes(item.id)} onToggle={() => toggleQueueItem(item.id)} onAction={queueAction} />)}{!visibleQueue.length && <p className="muted">لا توجد عناصر مطابقة للبحث الحالي.</p>}</div></section>
    </section>}
