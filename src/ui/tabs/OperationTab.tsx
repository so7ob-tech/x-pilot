...
export function OperationTab({ state, session, currentItem, logoUrl, activeWorkspaceName, published, failed, remaining, progress, countdownSeconds, canPause, canResume, preflight, settings, scheduleAt, setScheduleAt, start, stop, schedule, act }: OperationTabProps) {
  ...
  return <section className="tab-panel" role="tabpanel" aria-label={t('nav.operation')}>
    <section className="card operation-hero"><div className="hero-copy"><span className="eyebrow">{t('common.publishingControlCenter')}</span><h2>{t('operation.title')}</h2><p>{activeWorkspaceName ?? 'Workspace الحالية'} · {session?.status === 'RUNNING' ? t('operation.engineRunning') : t('operation.readyToRun')}</p></div><div className="hero-metric"><strong>{published}<small>/{state.queue.length}</small></strong><span>{t('operation.published')}</span></div><ProgressBar value={progress} label={t('operation.published')} /><div className="stats compact-stats"><MetricCard label={t('operation.remaining')} value={remaining} tone="primary" /><MetricCard label={t('operation.failed')} value={failed} tone={failed ? 'danger' : 'neutral'} /><MetricCard label={t('operation.skipped')} value={state.queue.filter((item) => item.status === 'SKIPPED').length} /></div>{session?.status === 'SCHEDULED' && session.scheduledStartAt && <div className="countdown dashboard-countdown"><span>{t('operation.startAt')}</span><strong>{new Date(session.scheduledStartAt).toLocaleString('ar')}</strong></div>}{session?.status === 'WAITING' && <div className="countdown dashboard-countdown"><span>{t('operation.nextTweetAfter')}</span><strong>{formatCountdown(countdownSeconds)}</strong></div>}</section>
    ...
    <section className="card controls"><h2>{t('nav.operation')}</h2><div className="row controls-row"><button className="primary" onClick={start} disabled={!state.queue.length || canPause || canResume || session?.status === 'SCHEDULED' || preflight === null || !preflight.ready}>{t('actions.start')}</button><button onClick={() => void act({ type: 'PAUSE' }, 'تم إيقاف Queue مؤقتًا')} disabled={!canPause}>{t('actions.pause')}</button><button onClick={() => void act({ type: 'RESUME' }, 'تم استئناف Queue')} disabled={!canResume}>{t('actions.resume')}</button><button className="danger" onClick={stop} disabled={!session || session.status === 'STOPPED' || session.status === 'COMPLETED'}>{session?.status === 'SCHEDULED' ? 'Cancel Schedule' : 'Stop'}</button></div><div className="schedule-controls"><label>{t('ui.startAt')}<input type="datetime-local" value={scheduleAt} onChange={(event) => setScheduleAt(event.target.value)} /></label><div className="row controls-row"><button onClick={() => schedule(false)} disabled={!state.queue.length || !scheduleAt || canPause || canResume}>{t('ui.schedule')}</button><button onClick={() => schedule(true)} disabled={session?.status !== 'SCHEDULED' || !scheduleAt}>{t('ui.reschedule')}</button></div></div><p className="muted">العنصر الحالي: {session?.currentItemId ? currentItem?.position ?? '-' : '-'}</p>{session?.status === 'PAUSED' && <p className="paused-hint">Queue متوقف مؤقتًا — اضغط Resume للمتابعة.</p>}</section>
  </section>;
}
