...
export interface QueueItem {
  id: string; workspaceId?: string; sourceBankUrl: string; targetUrl: string; label?: string; position: number;
...
export interface AutomationSession {
  id: string; workspaceId?: string; bankUrl: string; status: SessionStatus; currentItemId?: string; currentIndex: number; total: number;
...
export interface TweetBank { id: string; workspaceId: string; name: string; url: string; createdAt: number; updatedAt: number; lastExtractedAt?: number; lastExtractedCount?: number; }
...
export type RuntimeMessage =
  ...
  | { type: 'SET_ACTIVE_WORKSPACE'; workspaceId: string } | { type: 'GET_RUNTIME_STATUS' }
