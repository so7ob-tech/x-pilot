...
function DiffReview({ diff, selectedIds, onToggle, onAdd, onDiscard }: { diff: BankDiffResult; selectedIds: string[]; onToggle: (id: string) => void; onAdd: () => void; onDiscard: () => void }) {
  ...
  return <section className="diff-review"><div className="section-heading"><div><span className="eyebrow">REFRESH & DIFF</span><h3>مراجعة تغييرات البنك</h3></div><button onClick={onDiscard}>إغلاق</button></div><div className="diff-counts"><span>جديد: {counts.NEW ?? 0}</span><span>موجود: {counts.EXISTING ?? 0}</span><span>منشور سابقًا: {counts.PREVIOUSLY_PUBLISHED ?? 0}</span><span>محذوف: {counts.REMOVED ?? 0}</span><span>غير صالح: {counts.INVALID ?? 0}</span></div><div className="diff-items">{diff.items.map((item) => item.category === 'NEW' ? <label className="diff-item" key={item.id}><input type="checkbox" checked={selectedIds.includes(item.id)} onChange={() => onToggle(item.id)} /><span><strong>جديد</strong><span dir="auto">{getTweetPreview(item.url, item.label, 120)}</span><small dir="ltr">{item.url}</small></span></label> : <div className={`diff-item diff-${item.category.toLowerCase()}`} key={item.id}><span><strong>{diffCategoryLabel(item.category)}</strong><span dir="auto">{getTweetPreview(item.url, item.label, 120)}</span><small dir="ltr">{item.url}</small></span></div>)}{!diff.items.length && <p className="muted">لم تتغير محتويات البنك.</p>}</div><div className="diff-actions"><button className="primary" onClick={onAdd} disabled={!selectedIds.length}>إضافة المحدد إلى Queue</button><button onClick={onDiscard}>تجاهل التغييرات</button></div></section>;
}
